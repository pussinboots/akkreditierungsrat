...
class ModelSpec extends Specification with HSQLDbBefore {
  ...
  override def initTestData() {
    Studiengang.Insert(new Studiengang(None, Some(1), "fach", "abschluss", "hochschule", "bezugstyp", "link", None, 1))
    Studiengang.Insert(new Studiengang(None, Some(2), "fach2", "abschluss2", "hochschule2", "bezugstyp2", "link", Some("gutachtenlink"), 2))
  }

  "Studiengang class" should {
    "checksum should be the same" in {
      val studiengang = new Studiengang(None, Some(1), "fach", "abschluss", "hochschule", "bezugstyp", "link", None, 1)
