...
object StudiengangAttribute{
  ...
  def Insert(studiengangAttribute: StudiengangAttribute): StudiengangAttribute = {
    DB.withConnection {
      implicit connection =>
...
object Studiengang {
  val single = {
    get[Int]("studiengaenge.id") ~
      ...
      case id ~ fach ~ abschluss ~ hochschule ~ bezugstyp ~ link =>
        Studiengang(Option(id), fach, abschluss, hochschule, bezugstyp, link)
    }
  }

  def Insert(studiengang: Studiengang): Studiengang = {
    DB.withConnection {
      implicit connection =>
  ...
  //only work in hsqldb
  def Merge(studiengang: Studiengang): Studiengang = {
    DB.withConnection {
      implicit connection =>
        SQL(
