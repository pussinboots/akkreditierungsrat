...
class GenericSlickProvider[E <: Table[T], T](query: TableQuery[E]) extends SortableDataProvider[T, String] {
  ...
  def iterator(first: Long, count: Long): java.util.Iterator[T] = DB.db withDynSession {
    filter(query).sortBy(sortKey(_, getSort)).drop(first.toInt).take(count.toInt).list.iterator
  }

  def size: Long = DB.db withDynSession SlickQuery(filter(query).length).first
